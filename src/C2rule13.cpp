...
static real Weight[8][8] = {
    {0.2995235559387052215463143056692e+00,
    ...
    {7.610781847149629154716409791983e-2, 1.486101247399760261471935168346e-1,
     -2.077685631717747007172983323970e-1, 6.850758313011924198538315395405e-2,
     2.024205813317813585572881715385e-1, 1.108627473745508429879249169864e-1,
     -1.187411393304862640859204217487e-1,
     -5.208857468077715683772080394959e-2},
    {4.016494861405949747097510013162e-2, -1.093132962444079541048635452881e-1,
     -2.270251673633777452624380129694e-1, 1.231674163356097016086203579325e-2,
     -1.420402526499201540699111172200e-1, 1.189080551229557928776504129312e-1,
     -4.482039658150474743804189300793e-3, 1.730383808319875827592824151609e-1},
    {-5.643905795781771973971259866415e-1, 2.878418073676293225652331648545e-2,
     1.159354231997583294689565314470e-1, 1.376081498690624477894043101438e-1,
     -7.909780225340130915490382973570e-2, 1.174335441429478112778176601234e-1,
     -1.107251942334134124782600707843e-1, 2.094226883312045633400182488252e-2},
    {-2.269001713589584730602581694579e-1, 2.976190892690301120078774620049e-2,
     -7.440193483272787588251423144751e-2, -1.224665989043784131260454301280e-1,
     -4.857910454732976198562745578156e-2, 2.228157325962656425537280474671e-1,
     1.459764751457503859063666414952e-1, -1.211789553452468781539987084682e-1},
    {-3.326760468009974589269134283992e-1, 1.796655319904795478676993902115e-1,
     -4.389976396805911868560791966472e-2, -2.295841771339316497310760908889e-1,
     6.182618387692816082856552878852e-2, -1.202703885325137746461829140891e-1,
     5.109536580363550180208564374234e-3, 1.126062761533095493689566169969e-1},
    {2.290638530086106512999345512401e-1, 2.702070398116919449911037051753e-1,
     -9.078047988731123605988441792069e-3, 4.618480310858703283999169489655e-2,
     -2.598231009547631799096616255056e-1, -2.518433931146441037986247681820e-2,
     -1.257796993152456033984707367389e-2,
     -2.720818902721190304043617320910e-2},
    {2.746908885094872977794932213372e-1, -1.149427039769738298032807785523e-2,
     1.596178537820019535731955591283e-1, -2.180626972663360443142752377527e-1,
     -8.711748038292630173597899697063e-3, 1.902786182960269617633915869710e-1,
...
void Parallelogram_Rule13::ApplyWithDiffs(Integrand &F, Parallelogram &R,
                                          real &TheResult, real &TheError,
                                          Vector<real> &DiffOrder) {
  ...
  for (Type = 0; Type <= 3; Type++) {
    for (nr = 0; nr < K[Type]; nr++) {
      if (Type == 0) {
        ...
        z1 = Type2[nr];
        number = 4;
        x[1] = (-2 * R.Vertex(0) * z1 + R.Vertex(1) * (1 + z1) +
                R.Vertex(2) * (z1 + 1)) /
               2;
        x[2] = (2 * R.Vertex(0) * z1 + R.Vertex(1) * (1 - z1) +
                R.Vertex(2) * (1 - z1)) /
               2;
        ...
        z2 = Type3[nr][1];
        number = 8;
        x[1] = (R.Vertex(0) * (-z1 - z2) + R.Vertex(1) * (1 + z2) +
                R.Vertex(2) * (1 + z1)) /
               2;
        x[2] = (R.Vertex(0) * (+z1 - z2) + R.Vertex(1) * (1 + z2) +
                R.Vertex(2) * (1 - z1)) /
               2;
        x[3] = (R.Vertex(0) * (-z1 + z2) + R.Vertex(1) * (1 - z2) +
                R.Vertex(2) * (1 + z1)) /
               2;
        x[4] = (R.Vertex(0) * (+z1 + z2) + R.Vertex(1) * (1 - z2) +
                R.Vertex(2) * (1 - z1)) /
               2;
        x[5] = (R.Vertex(0) * (-z1 - z2) + R.Vertex(1) * (1 + z1) +
                R.Vertex(2) * (1 + z2)) /
               2;
        x[6] = (R.Vertex(0) * (-z1 + z2) + R.Vertex(1) * (1 + z1) +
                R.Vertex(2) * (1 - z2)) /
               2;
        x[7] = (R.Vertex(0) * (+z1 - z2) + R.Vertex(1) * (1 - z1) +
                R.Vertex(2) * (1 + z2)) /
               2;
        x[0] = (R.Vertex(0) * (+z1 + z2) + R.Vertex(1) * (1 - z1) +
                R.Vertex(2) * (1 - z2)) /
               2;
      }
...
//////////////////////////////////////////////////
Parallelogram_Rule13::Parallelogram_Rule13() : Rule<Parallelogram>() {}
//////////////////////////////////////////////////
} // namespace cubpackpp
