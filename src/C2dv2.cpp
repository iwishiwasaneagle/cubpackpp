...
namespace cubpackpp {
//////////////////////////////////////////////////
void Parallelogram_Divide2::Apply(const Parallelogram &t,
                                  Stack<Parallelogram> &Offspring,
                                  const Vector<unsigned int> &DiffOrder) {
  Point m12 = (t.Vertex(0) + t.Vertex(DiffOrder[0])) / 2;
  Point m34 =
      t.Vertex(DiffOrder[1]) + (t.Vertex(DiffOrder[0]) - t.Vertex(0)) / 2;

  Parallelogram *t1 =
      new Parallelogram(t.Vertex(0), m12, t.Vertex(DiffOrder[1]));
