...
static real Type1[] = {0.5460844152585403666984417e+00,
                       0.1376486679696350642323050e+01,
                       0.2358932261980680967828022e+01};
static real Type2[] = {0.8462499884480199068814807e+00,
                       0.1985531339917884990890422e+01};
static real Type3[2][2] = {
    {0.1779246738905319006374908e+01, 0.9318240227761998500475049e+00},
    {0.3030436065368579151664045e+01, 0.9962534261727632104611162e+00}};

static real OrigWeight[7] = {
    0.1418036987406628465242303e+00, 0.3807850175899473939094384e-01,
    0.1290856883945041937607418e-02, 0.5772571826901623901546648e-01,
    0.1620590503146543183518239e-03, 0.5445472363593369147167489e-02,
    0.2411028493987025953260209e-04};

static int d[] = {0, 3, 1, 5, 2, 4, 6};

static real Weight[7][7] = {
    {0.6002713316445080080142126, 0.7955990316249109629216728,
     1.0584888443837658614605899, 0.7595383160846588054989698,
     1.3523576929325600784468914, 0.9663531601396869556259704,
     1.9895758469520118113052969},
    {-0.1714120818975139, -1.4261615934658296, 20.9491066359021694,
     1.7029699666146943, 57.2013795938251143, -4.3332227154742827,
     -175.0624620496226842},
    {-0.2883800695213331, 2.5450105556734605, 2.8699527274413806,
     0.304671504643251, 138.1913876850488233, -8.5573830821954439,
     -18.7930973279112966},
    {-0.250456836419066, 0.3273926337592694, -83.9551965137636766,
     0.4609375440071548, -365.1974538111883157, 10.4127651350263225,
     1537.3605596124529503},
    {-0.2595143739988262, 0.1255865390202144, 85.4652862386879934,
     -0.2489288730337109, -712.5513360902265439, 6.3018929219370941,
     -645.6559424743654782},
    {-0.24820543230912, -0.2143965818640334, 8.5337065366470588,
     -0.2238393816932916, 867.6556565044116888, 7.6084037588845301,
     -4712.804729866979117},
    {0.3052968227487744, 1.0011209710514362, -6.3357177267870899,
     0.7227067892680122, -358.0249035190603089, 2.7469614313645364,
     -6407.2910382311609777}};

// Radii of rings associated with weights of published formula
static real RingRadius[] = {
    0.9151577658134376, 1.2649388389767648, 1.7333214530567136,
    2.2616892861784437, 2.6609731353556634, 2.9246248487342039,
...
//////////////////////////////////////////////////////
void PlaneAdaptive::Rule(Integrand &F, Plane &R, real &TheResult,
  ...
  for (Type = 0; Type <= 3; Type++) {
    for (nr = 0; nr < K[Type]; nr++) {
      if (Type == 0) {
        ...
        Point z1(A * Type1[nr], 0), z2(0, B * Type1[nr]);
        number = 4;
        x[0] = R.Center() + z1;
        x[1] = R.Center() - z1;
        x[2] = R.Center() + z2;
...
//////////////////////////////////////////////
void PlaneAdaptive::Process(Stack<AtomicRegion> &Offspring) {
  TimesCalled++;
  if (TimesCalled == 1) {
    Rule(LocalIntegrand(), Geometry(), Integral(), AbsoluteError(),
         HalfValueRadius);
  } else {
...
Processor<Plane> *PlaneAdaptive::NewCopy() const {
